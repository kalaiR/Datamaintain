...
def staff_details(request):
    staff_details.username=request.POST.get('username','')
    staff_details.password=request.POST.get('password','')
    staff_details.username=request.POST.get('username','')
    department=Department.objects.all()
    staff_details.specialization=request.POST.get('specialization','')
    year=Year.objects.all()
    staff_details.save()
    
    return render_to_response('datamaintain/staff_details.html', context_instance=RequestContext(request))

def parent_details(request):
    parent_details.fathername=request.POST.get('fathername','')
    parent_details.mothername=request.POST.get('mothername','')
    parent_details.address=request.POST.get('address','')
    parent_details.email=request.POST.get('email','')
    parent_details.phone_no=request.POST.get('phone_no','')
    student_id=Student_Id.objects.all()
    parent_details.save()
    
    
    return render_to_response('datamaintain/parent_details.html', context_instance=RequestContext(request))
